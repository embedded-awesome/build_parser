...
class _UnresolvedRef(NamedTuple):
    ref: str
    label: Optional[str]
    node: Node
...
class DT:
    ...
    def __init__(self, filename: Optional[str], include_path: Iterable[str] = (),
        ...
        self.unresolved_refs: list[_UnresolvedRef] = []
    ...
    def __deepcopy__(self, memo):
        ...
        ret.unresolved_refs = [
            _UnresolvedRef(
                unresolved.ref,
                unresolved.label,
                self._copy_detached_node(unresolved.node, ret),
            )
            for unresolved in self.unresolved_refs
        ]
    ...
    def _copy_detached_node(self, node, dt_copy):
        node_copy = Node(node.name, None, dt_copy, node.filename, node.lineno)
        ...
        for prop_name, prop_copy in prop_name2prop_copy.items():
            ...
            prop_copy.filename = prop.filename
            prop_copy.lineno = prop.lineno
            if hasattr(prop, "_cell_items"):
                prop_copy._cell_items = prop._cell_items[:]
        node_copy.props = prop_name2prop_copy

        node_copy.nodes = {
            child_name: self._copy_detached_node(child_node, dt_copy)
            for child_name, child_node in node.nodes.items()
        }
        for child_node in node_copy.nodes.values():
            child_node.parent = node_copy
    ...
    def _new_unresolved_ref_node(self, ref):
        return Node(
            name=f"unresolved-ref-{len(self.unresolved_refs)}",
            parent=None,
            dt=self,
        ...
        while True:
            ...
            if tok.val == "/":
                ...
                try:
                    ...
                    if tok.val[0] == "{" or not str(e).startswith("undefined node label"):
                        self._parse_error(e)

                    node = self._new_unresolved_ref_node(tok.val)
                    self._parse_node(node)
                    self.unresolved_refs.append(_UnresolvedRef(tok.val, label, node))
                    continue
                self._parse_node(node)
    ...
    def _parse_cells(self, prop, n_bytes):
        ...
        while True:
            ...
            if tok.id == _T.REF:
                ...
                # Literal value
                num = self._eval_prim()
                prop._cell_items.append(num)
                if self._is_symbolic_expr(num):
                    # Keep byte length stable while preserving symbolic source
                    # text in prop._cell_items for downstream serializers.
                    prop.value += (0).to_bytes(n_bytes, "big")
                    continue
                try:
    ...
    @staticmethod
    def _is_symbolic_expr(val):
        return isinstance(val, str)

    @staticmethod
    def _expr_to_text(val):
        return val if isinstance(val, str) else str(val)

    def _unary_expr(self, op, val):
        if self._is_symbolic_expr(val):
            return f"{op}{self._expr_to_text(val)}"
    ...
    def _binary_expr(self, lhs, rhs, op):
        if self._is_symbolic_expr(lhs) or self._is_symbolic_expr(rhs):
            return f"({self._expr_to_text(lhs)} {op} {self._expr_to_text(rhs)})"

        if op == "||":
            return 1 if rhs or lhs else 0
        if op == "&&":
            return 1 if rhs and lhs else 0
        if op == "|":
            return lhs | rhs
        if op == "^":
            return lhs ^ rhs
    ...
    def _collect_parenthesized_tail(self):
        # Assumes '(' has already been consumed. Returns a token-based string
        ...
        while depth:
            tok = self._next_token()
            if tok.id == _T.EOF:
                self._parse_error("unterminated parenthesized expression")
            if tok.val == "(":
                depth += 1
            elif tok.val == ")":
                depth -= 1
            parts.append(self._expr_to_text(tok.val))
    ...
    def _eval_prim(self):
        ...
        if tok.id == _T.IDENT:
            ident = self._next_token().val
            if self._check_token("("):
                return ident + self._collect_parenthesized_tail()
            return ident
    ...
    def _eval_ternary(self):
        ...
        if self._check_token("?"):
            ...
            if (self._is_symbolic_expr(val)
                or self._is_symbolic_expr(if_val)
                or self._is_symbolic_expr(else_val)):

                return (
                    f"{self._expr_to_text(val)} ? "
                    f"{self._expr_to_text(if_val)} : "
                    f"{self._expr_to_text(else_val)}"
                )
    ...
    def _eval_or(self):
        ...
        while self._check_token("||"):
            val = self._binary_expr(val, self._eval_and(), "||")
        return val

    def _eval_and(self):
        val = self._eval_bitor()
        while self._check_token("&&"):
            val = self._binary_expr(val, self._eval_bitor(), "&&")
        return val

    def _eval_bitor(self):
        val = self._eval_bitxor()
        while self._check_token("|"):
            val = self._binary_expr(val, self._eval_bitxor(), "|")
        return val

    def _eval_bitxor(self):
        val = self._eval_bitand()
        while self._check_token("^"):
            val = self._binary_expr(val, self._eval_bitand(), "^")
        return val

    def _eval_bitand(self):
        val = self._eval_eq()
        while self._check_token("&"):
            val = self._binary_expr(val, self._eval_eq(), "&")
        return val
    ...
    def _eval_eq(self):
        ...
        while True:
            if self._check_token("=="):
                val = self._binary_expr(val, self._eval_rela(), "==")
            elif self._check_token("!="):
                val = self._binary_expr(val, self._eval_rela(), "!=")
            else:
                return val
    ...
    def _eval_rela(self):
        ...
        while True:
            if self._check_token("<"):
                val = self._binary_expr(val, self._eval_shift(), "<")
            elif self._check_token(">"):
                val = self._binary_expr(val, self._eval_shift(), ">")
            elif self._check_token("<="):
                val = self._binary_expr(val, self._eval_shift(), "<=")
    ...
    def _eval_shift(self):
        ...
        while True:
            if self._check_token("<<"):
                val = self._binary_expr(val, self._eval_add(), "<<")
            elif self._check_token(">>"):
                val = self._binary_expr(val, self._eval_add(), ">>")
            else:
                return val
    ...
    def _eval_add(self):
        ...
        while True:
            if self._check_token("+"):
                val = self._binary_expr(val, self._eval_mul(), "+")
            elif self._check_token("-"):
                val = self._binary_expr(val, self._eval_mul(), "-")
            else:
                return val
    ...
    def _eval_mul(self):
        ...
        while True:
            if self._check_token("*"):
                val = self._binary_expr(val, self._eval_unary(), "*")
            elif self._check_token("/"):
                val = self._binary_expr(val, self._eval_unary(), "/")
            elif self._check_token("%"):
                val = self._binary_expr(val, self._eval_unary(), "%")
    ...
    def _eval_unary(self):
        if self._check_token("-"):
            return self._unary_expr("-", self._eval_unary())
        if self._check_token("~"):
            return self._unary_expr("~", self._eval_unary())
        if self._check_token("!"):
            return self._unary_expr("!", self._eval_unary())
        ...
        while True:
            ...
            if match:
                ...
                if match:
                    ...
                    match = _ident_re.match(self._file_contents, self._tok_end_i)
                    if match:
                        tok_id = _T.IDENT
